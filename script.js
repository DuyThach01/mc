...
function createGift() {
  ...
  const messages = [
    ...
    '🎄 Giáng sinh an lành!',
    '⭐ Luôn mạnh khỏe, hạnh phúc nè!',
    '🎅 Hí hí ,1 2 3 cười lên nào ',
    '🎄 Luôn xinh đẹp nhé ',
  ];
...
function createFirework(x, y) {
  ...
  // Animation đã được định nghĩa trong CSS

  // Xóa phần tử sau khi animation kết thúc
  setTimeout(() => {
    firework.remove();
  }, 500); // 500ms tương ứng với thời gian của animation
}

// Hàm tạo Firework Particles (đổi tên để tránh trùng lặp)
...
// Hàm tạo các hiệu ứng ngẫu nhiên tự động
function startRandomEffects() {
  function createRandomEffects() {
    const numberOfEffects = Math.floor(Math.random() * 2) + 2; // 2 hoặc 3 hiệu ứng mỗi lần

    for (let i = 0; i < numberOfEffects; i++) {
      const x = Math.random() * window.innerWidth;
      const y = Math.random() * window.innerHeight;

      // Tạo pháo hoa và particles
      createFireworkParticles(x, y);
      createFirework(x, y);
    }
  }

  // Thiết lập interval để gọi hàm tạo hiệu ứng ngẫu nhiên mỗi giây
  setInterval(createRandomEffects, 1000); // 1000ms = 1 giây
}
// Hàm thêm tương tác với cây thông
function addTreeInteraction() {
  const tree = document.querySelector('.tree');
  const bells = document.querySelectorAll('.bell');

  tree.addEventListener('click', () => {
    // Chờ 1 giây trước khi thực hiện lắc
    setTimeout(() => {
      ...
      bells.forEach((bell) => {
        ...
        bell.offsetHeight; // Kích hoạt reflow để đảm bảo animation được reset
        bell.style.animation = 'bellRing 0.5s';
      });

      // Sau khi hiệu ứng lắc kết thúc (0.5 giây)
      setTimeout(() => {
        tree.classList.remove('shake');
        bells.forEach((bell) => {
          bell.style.animation = 'bellRing 2s infinite';
        });
      }, 500); // 500ms tương ứng với thời gian của hiệu ứng lắc
    }, 1000); // 1000ms = 1 giây trước khi bắt đầu lắc
  });
}

// Hàm trang trí cây thông
...
// Hàm khởi tạo các hiệu ứng khi trang đã load
document.addEventListener('DOMContentLoaded', () => {
  ...
  animateClouds();
  addTreeInteraction();
  startRandomEffects(); // Thêm hiệu ứng ngẫu nhiên

  // Các sự kiện khác đã được tích hợp vào 'click' event listener
  document.addEventListener('click', (e) => {
    createFirework(e.pageX, e.pageY);
    createFireworkParticles(e.pageX, e.pageY);
    createMouseParticle(e);
  });

  // Sự kiện di chuột tạo particle (giữ nguyên)
